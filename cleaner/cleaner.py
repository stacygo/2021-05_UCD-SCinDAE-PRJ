...
if __name__ == '__main__':
    ...
    find_total_mark = df['criteria'] == 'TOTAL MARK'
    output_df = \
        df[find_total_mark].pivot_table(index=['county'], columns=['year'], values='max_mark',
    ...
    df['date'] = df['date'].fillna(pd.to_datetime({'year': df['year'], 'month': 9, 'day': 1}))

    # Drop some columns, and drop duplicates
    choose_cols = ['county', 'town', 'criteria', 'pdf_name', 'pdf_path', 'pdf_path_county']
